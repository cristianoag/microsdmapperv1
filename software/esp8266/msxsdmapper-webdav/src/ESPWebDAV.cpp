...
const char *basehtml =
	"<html><head><title>MSX MicroSDMapper Setup</title></head><body>"
	"<h1>The Retro Hacker MicroSDMapper</h1>"
	"WiFi Connection Setup Form<br><br>"
...
// ------------------------
	...
	if (ret) {
		...
		file.open("/ip.cfg", O_WRITE | O_CREAT | O_TRUNC);
		file.print(WiFi.localIP().toString());
		file.close();
		DBG_PRINTLN("\nIP.CFG file created");
	}
...
// ------------------------
	...
	if (method.equals("GET") && uri.equals("/")) {
		...
		html.replace("{host}", config.hostname_flag == 9 ? config.hostname : "msxsdmapper");
